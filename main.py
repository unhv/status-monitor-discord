...
def find_channel_by_fuzzy_name(guild, fuzzy_name, cutoff=0.6):
    ...
    matches = difflib.get_close_matches(fuzzy_name.lower(), [name.lower() for name in channel_names.keys()], n=1, cutoff=cutoff)
    if matches:
...
async def on_ready():
    ...
    guild = bot.guilds[0]  # Assumes one guild/server

    # Fuzzy match monitor channels
    monitor_channels = []
    for name in monitor_channel_names:
        ch = find_channel_by_fuzzy_name(guild, name)
        if ch:
            monitor_channels.append(ch)
            print(f"✅ Matched: '{name}' → '{ch.name}'")
        else:
            print(f"❌ No match for: {name}")

    # Fuzzy match status channel
